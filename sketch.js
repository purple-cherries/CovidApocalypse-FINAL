...
canvas = createCanvas(displayWidth, displayHeight-150)
   ...
   Background1.scale = 2
    ...
    check1 = createSprite(displayWidth-200,150,10,10)
    check1.addImage(check)
    check1.scale = 0.3

    check2 = createSprite(displayWidth-200,250,10,10)
    check2.addImage(check)
    check2.scale = 0.3

    check3 = createSprite(displayWidth-200,350,10,10)
...
function draw(){
  ...
  if(gameState==="Intro-Level2"){
    ...
    textSize(30)
    text("Use Space key to collect the leaking vaccine!.",displayWidth/2,250)
    text("Use Arrow keys to move the barrel",displayWidth/2,350)
    text("Click on next button to continue.",displayWidth/2,450)
...
function cars(){
  if(frameCount%120===0){
    var y = random(100,displayHeight-400)
